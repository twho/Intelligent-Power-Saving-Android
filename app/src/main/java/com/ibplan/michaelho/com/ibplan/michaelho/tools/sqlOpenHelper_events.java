...
public class sqlOpenHelper_events extends SQLiteOpenHelper implements EventConstants{
    ...
    public static final String DBNAME = "campuseventdb.sqlite";
    public static final int VERSION = 1;
    public static final String TABLENAME = "table_campus_events";
    ...
    public long insertDB(Event event) {
        ...
        values.put(POSTERIMG, event.getPosterImg());
        values.put(TIME, event.getTime());
        values.put(IFFIXED, event.getIfFixed());
    ...
    public Event getSingleEvent(int id) {
        SQLiteDatabase db = getWritableDatabase();
        String[] columns = { STUDENT_NAME, DEPARTMENT, LOCATION, X_POS, Y_POS, EVENTS, TIME, IFFIXED};
        ...
        byte[] posterImg = cursor.getBlob(8);
        String time = cursor.getString(9);
        String if_fixed = cursor.getString(10);
        Event event = new Event(name, department, location, x_pos, y_pos, events, image, posterImg, time, if_fixed);
        return event;
    }
    ...
    public ArrayList<HashMap<String, Object>> getAllEvents() {
        ...
        while (cursor.moveToNext()) {
            ...
            item.put(POSTERIMG, cursor.getBlob(8));
            item.put(TIME, cursor.getString(9));
            item.put(IFFIXED, cursor.getString(10));
            events.add(item);
        }
